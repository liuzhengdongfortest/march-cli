...
- When renaming or re-describing an existing memory, update the memory metadata fields (`name`, `description`, `tags`) as metadata, not only the Markdown body heading. The recall title comes from memory metadata/indexing, and already-injected recall hints in the current prompt may remain stale until the next recall.
