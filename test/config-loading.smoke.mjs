...
export async function runConfigLoadingSmoke({ setupTmp, cleanup }) {
  ...
  assert.deepEqual(empty.notifications, { turnEnd: false });

  writeFileSync(join(dir, ".marchrc"), JSON.stringify({ model: "test-model", memoryRoot: "D:/vault/March Memories", skills: ["ignored-legacy-skill"], notifications: { turnEnd: true }, webSearch: { provider: "tavily", providers: { tavily: { apiKey: "tvly" } } } }));
  ...
  assert.deepEqual(withRc.notifications, { turnEnd: true });
