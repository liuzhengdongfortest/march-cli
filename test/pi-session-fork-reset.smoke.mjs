...
export async function runPiSessionForkResetSmoke({ setupTmp, cleanup }) {
  ...
  const engine = new ContextEngine({ cwd: dir, modelId: "test", provider: "deepseek", thinkingLevel: "high", namespace: "ns" });
  ...
  assert.equal(Object.hasOwn(sidecar.state, "skills"), false);


  let rolledBackTo = null;
  const rollbackEngine = new ContextEngine({ cwd: dir, modelId: "test", provider: "deepseek", namespace: "ns" });
