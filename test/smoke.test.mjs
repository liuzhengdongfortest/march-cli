...
import { runLoggerSmoke } from "./logger.smoke.mjs";
import { runLspSmoke } from "./lsp.smoke.mjs";
...
await runLoggerSmoke({ setupTmp, cleanup });
await runLspSmoke({ setupTmp, cleanup });
...

{
  ...
  assert.equal(commands.find((command) => command.name === "session").description, "Open workspace session selector");
  assert.equal(commands.find((command) => command.name === "save").description, "Show auto-save status");
  assert.ok(!commands.some((command) => command.name === "switch"));
