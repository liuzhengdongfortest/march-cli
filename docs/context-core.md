...
**排序原因**：稳定前缀更容易命中 provider prefix cache；大块且相对稳定的内容尽量放在高频小层之前。文件正文不再作为常驻层注入，AI 需要时通过 `read` 读取；记忆召回结果不作为独立层注入，而是跟随消息附加在尾部的 [recent_chat] 中。技能系统不再作为独立上下文层存在，原技能类方法论迁移为 memory 文档，由 memory hint 提示线索，再通过 `memory_open` 主动读取正文。
...
- **边界**：不包含目录结构、git 状态、最近变更文件；需要工作区事实时由 AI 通过 `glob` / `grep` / `git status` 按需查询
...
3. [recent_chat] — 注入最近 10 个 turn 摘要，以及已附加在消息后的 recall hints
4. 组装完成 → 发给 AI
...
1. 用户消息进入 turn 时，March 检查当前 shell runtime 是否有非 killed shell
2. 如果有，只附加 `[shell_hints]` 短摘要，不附加 shell 输出
