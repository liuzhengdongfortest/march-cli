...
**排序原因**：稳定前缀更容易命中 provider prefix cache；大块且相对稳定的内容尽量放在高频小层之前。文件正文不再作为常驻层注入，AI 需要时通过 `read` 读取；记忆召回结果不作为独立层注入，而是跟随消息附加在尾部的 [recent_chat] 中。
...
5. [diagnostics] — 读取当前 LSP/编译器/linter 诊断状态，注入相关错误摘要
6. [workspace_status] — March 写入目录结构摘要、git 状态和最近工作区变化
7. [runtime_status] — March 写入当前时间和压力指标
8. [shells] — 从 PTY runtime 读取当前 shell 输出摘要
9. [recent_chat] — 注入最近 10 个 turn 摘要，以及已附加在消息后的 recall hints
10. 组装完成 → 发给 AI
