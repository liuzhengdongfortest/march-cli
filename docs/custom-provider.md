...
March supports custom OpenAI-compatible model providers through `.march/config.json`. March reads the config, registers the provider in pi's model registry, then the normal model selection path stays unchanged.
...
- `~/.march/config.json` for global defaults
- `<project>/.march/config.json` for one project
...
```json
{
  "providers": {
    "local-openai": {
      ...
      "models": [
        { "id": "qwen2.5-coder", "contextWindow": 32768, "maxTokens": 4096 }
      ]
    }
  },
